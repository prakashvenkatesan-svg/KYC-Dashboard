const API_BASE_URL = 'https://1xhidsu4ra.execute-api.ap-south-1.amazonaws.com/admin/kyc-dashboard';
...
const parseApiResponse = async (res, fallbackMessage) => {
  ...
  if (res.status === 204) {
    return { success: true, message: fallbackMessage || 'Success' };
  }
  ...
  return {
    success: res.ok,
    message: text && text.trim() ? text.trim() : (fallbackMessage || 'Request failed.')
  };
};
...
const api = {
  ...
  getPushOperationsReport: async () => {
    const res = await fetch(`${API_BASE_URL}/push-operations-report`, { headers: getAuthHeaders() });
    if (res.status === 403) {
      return { success: false, forbidden: true, message: 'Admin access required' };
    }
    if (!res.ok) throw new Error('Failed to fetch push operations report');
    return res.json();
  },
  ...
  getClientById: async (appId) => {
    const res = await fetch(`${API_BASE_URL}/kyc-applications/${appId}/details`, { headers: getAuthHeaders() });
    if (res.status === 401) {
      localStorage.removeItem('kyc_auth_token');
      window.location.href = 'login.html';
      return;
    ...
    if (!res.ok && res.status !== 401 && res.status !== 403) {
      ...
      throw new Error('Failed to fetch client detail: ' + errText);
    }
    return res.json();
  },
  ...
  getClientKycStages: async (applicationId) => {
    const res = await fetch(`${API_BASE_URL}/kyc-applications/${applicationId}/stages`, { headers: getAuthHeaders() });
    if (res.status === 401) {
      localStorage.removeItem('kyc_auth_token');
      window.location.href = 'login.html';
      return;
    }
    if (!res.ok) throw new Error('Failed to fetch client KYC stages');
    return res.json();
  },
  getPayments: async (params = {}) => {
    const qs = new URLSearchParams(params).toString();
    const res = await fetch(`${API_BASE_URL}/payments?${qs}`, { headers: getAuthHeaders() });
    if (res.status === 401) {
      localStorage.removeItem('kyc_auth_token');
      window.location.href = 'login.html';
      return;
  ...
  get: async (path) => {
    ...
    return parseApiResponse(res, 'Request completed.');
  },
  ...
  delete: async (path) => {
    ...
    return parseApiResponse(res, 'Deleted successfully.');
  }
};
