  ...
  try {
    const result = await pool.query(
      `SELECT id, full_name, email_id, mobile_number, username, role, account_status, assigned_modules, created_at, last_login 
       FROM public.admin_users ORDER BY id ASC`
    );
...
const createUser = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO public.admin_users (full_name, email_id, mobile_number, username, password_hash, role, account_status, assigned_modules)
...
const deleteUser = async (req, res) => {
  try {
    ...
    const result = await pool.query(`DELETE FROM public.admin_users WHERE id = $1`, [userId]);
